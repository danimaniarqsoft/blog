document.addEventListener('DOMContentLoaded', function() {
  if (window.wordCloudTagsData && document.getElementById('word-cloud-tags-container')) {
    const data = window.wordCloudTagsData;
    const container = document.getElementById('word-cloud-tags-container');
    ...
    function draw(words) {
      ...
      d3.select("#word-cloud-tags-container").append("svg")
        ...
        .on("click", function(event, d) {
          ...
          window.location.href = `/blog/tags/${encodeURIComponent(d.text.toLowerCase()).replace(/%20/g, '-')}/`;
        })
